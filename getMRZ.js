...
const ERR_NO_ROI = 'ERR_NO_ROI';

module.exports = async function(buffer) {
    const image = await Image.load(buffer);
    const mrz = await getMrz(image);

    const filename = `cropped-${uuidv4()}.jpg`;

    await mrz.save(`./tmp/${filename}`);

    return { filename, roi: mrz.roi };
}
...
function internalGetMrz(image, options = {}) {
    ...
    let mrz = toCrop.crop(mrzCropOptions);

    mrz.roi = mrzRoi;

    if (debug) images.crop = cropped;

    return debug ? { images } : mrz;
}
